...
with tempfile.TemporaryDirectory() as tmp:
    ...
    result = subprocess.run(["python3", str(ROOT / "scripts" / "generate.py")], env=env, capture_output=True, text=True, check=True)
    ...
    # start.sh creates these persistent runtime values before backup_state.py.
    state = {
    ...
    rotated = subprocess.run(["python3", str(ROOT / "scripts" / "rotate_subscription_token.py"), str(data)], env=env, capture_output=True, text=True, check=True)
    assert "subscription token rotated" in rotated.stdout
    assert "old-token" not in rotated.stdout
    assert (data / "subscription_token.txt").read_text().strip() != "old-token"
    assert "/sub/" in (data / "subscription_url.txt").read_text()

    restored = tmp / "restored"
    restored.mkdir()
    restored_config = restored / "config.json"
    subprocess.run(["python3", str(ROOT / "scripts" / "restore_state.py"), str(restored), str(backups[0]), str(restored_config)], env=env, check=True)
    for name, value in state.items():
